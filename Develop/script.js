...
const collectEmployees = function() {
  const employeesInfo=[];
  do {
    const firstName = prompt("Type the employee\u0027s first name.");
    const lastName = prompt("Type the employee\u0027s last name.");
    let salary = parseInt(prompt("Type the employee\u0027s salary."));
    if (isNaN(salary)) {
      salary = 0;
    }
...
const displayAverageSalary = function(employeesArray) {
  ...
  let averageSalary = (employeesArray.reduce((acc, employeesArray) => acc + employeesArray.salary, 0) / employeesArray.length);
